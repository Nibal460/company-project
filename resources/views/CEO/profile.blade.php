...
<body>
    <div class="container">
        ...
        <p style="font-size: 18px;"><strong>First Name:</strong> {{ $ceo->first_name }}</p>
        <p style="font-size: 18px;"><strong>Last Name:</strong> {{ $ceo->last_name }}</p>
        <p style="font-size: 18px;"><strong>Email:</strong> {{ $ceo->email }}</p>
        <p style="font-size: 18px;"><strong>Gender:</strong> {{ $ceo->gender }}</p>
        @if($ceo->qr_code_path)
            <p><strong>QR Code:</strong></p>
            <img src="{{ asset('storage/' . $ceo->qr_code_path) }}" alt="QR Code">
        @endif

        <form id="logout-form" action="{{ route('ceo.logout') }}" method="POST" style="display: none;">
            @csrf
        </form>
        ...
        <div class="custom-search-input">
            <div class="input-group">
                <input type="text" id="search" class="form-control" placeholder="Search Trainees...">
            ...
            <tbody id="students-list">
                @foreach($students as $student)
                    <tr>
                        ...
                        <a href="{{ route('ceo.users.show', $student->id) }}" class="btn btn-sm btn-secondary">Show</a>
